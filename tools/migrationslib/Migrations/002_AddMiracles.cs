...
[Migration(2)]
public class AddMiracles : Migration
{
    public override void Up()
    {
        ...
        // Ensure lookup index for name searches
        ...
        Execute.WithConnection((conn, tran) =>
        {
            ...
            using (var check = conn.CreateCommand())
            {
                ...
                check.CommandText = "SELECT COUNT(*) FROM miracles;";
...
VALUES (@id, @name, @name_lower, @power, @sphere, @alignment, @description, @is_advanced, @data_json, @created_at, @updated_at);";
    ...
    {
        Execute.Sql("DROP INDEX IF EXISTS idx_miracles_name_lower;");
        if (Schema.Table("miracles").Exists())
        {
            Delete.Table("miracles");
        }
    }
