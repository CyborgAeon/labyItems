...
namespace labyItems.Controls
{
    ...
    {
        // ---------------- Bindable properties ----------------
// NEW: extra argument for KeyFormat’s {1}
public static readonly BindableProperty FormatArg1Property =
...
public object? FormatArg1
{
    ...
    set => SetValue(FormatArg1Property, value);
        ...
        public static readonly BindableProperty MinCountProperty =
            BindableProperty.Create(
                nameof(MinCount),
                typeof(int),
                typeof(DictionarySlider),
        ...
        public static readonly BindableProperty MultiplierProperty =
            BindableProperty.Create(
                ...
                typeof(int),
                typeof(DictionarySlider),
                defaultValue: 4, // General store default; set to 3 for Sphere
                propertyChanged: OnGeneratedParamsChanged);
...
{
    if (fmt is null) return string.Empty;
    try { return string.Format(fmt ?? "{0}", args); }
    catch { return args is { Length: >0 } ? $"{args[0]}" : string.Empty; }
}
        ...
        {
            var control = (DictionarySlider)bindable;
            if (control._usingGenerated) return; // index not used in generated mode
        ...
        {
            var control = (DictionarySlider)bindable;
            if (!control._usingGenerated) return; // count not used in dictionary mode
        ...
        private void UpdateFromGeneratedOrIndex(bool raiseEvent, int? previousIndex = null)
        {
            ...
            if (_usingGenerated)
            {
                ...
                KeyLabel.Text   = SafeFormat(KeyFormat, kv.Key, FormatArg1);
                ValueLabel.Text = SafeFormat(ValueFormat, kv.Value);

                if (raiseEvent)
                    SelectionChanged?.Invoke(this, new DictionarySelectionChangedEventArgs(
                        previousIndex ?? count, count, kv.Key, kv.Value));
            }
            else
            {
                ...
                if (raiseEvent)
                    SelectionChanged?.Invoke(this, new DictionarySelectionChangedEventArgs(
                        previousIndex ?? idx, idx, kv.Key, kv.Value));
            }
        }
